...
def load_and_prepare_data(file_path):
    ...
    # Initialisation
    df_raw_eu, df_ciment, df_electro_agg, df_electro_detail, df_electro_size_power = None, None, None, None, None
    
    # Création du DataFrame de substitution pour Electroniques3 en cas d'échec
    data_substitute = {
    ...
    try:
        ...
        # Chargement des données pour le nuage de points
        df_electro_size_power = pd.read_excel(file_path, sheet_name="Electroniques3")
        
    except Exception as e:
        # Gestion spécifique de l'erreur pour Electroniques3 (si la feuille n'est pas trouvée)
    ...
    # --- Préparation Electroniques AGRÉGÉES (Bar Chart) ---
    ...
    # --- Préparation Electroniques DÉTAILLÉES (Pie Chart) ---
    ...
    # --- PRÉPARATION DU NUAGE DE POINTS (Electroniques3) ---
    if df_electro_size_power is not None and df_electro_size_power is not df_electro_size_power_sub:
        ...
        if all(col in df_electro_size_power.columns for col in required_cols_sp):
            ...
            st.error(f"Erreur de colonnes dans la feuille Electroniques3. Utilisation des données de substitution.")
            df_electro_size_power = df_electro_size_power_sub
    ...
    return df_eu, df_ciment, df_electro_agg, df_electro_detail, df_electro_size_power 

# Lancement du chargement
df_eu, df_ciment, df_electro_agg, df_electro_detail, df_electro_size_power = load_and_prepare_data(file_path)

if df_eu is None:
    st.stop() 

# --- 2. Définition des Onglets ---
